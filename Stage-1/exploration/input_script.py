...
class DroidBotScript(object):
    ...
    def parse_main(self):
        ...
        for state_id in script_value:
            ...
            self.main_list.append(state_selector)
    ...
    def is_all_operations_executed(self):
        for operation in self.operations:
            if self.operations[operation].executed == False:
                return False
        return True

    def get_operation_based_on_state(self, state, last_state):
        """
        ...
        # first sorted state_selector_keys including string "-a.b" and string 'b.a".
        original_state_selector_keys = self.states.keys()
        sorted_state_selector_keys = sorted(original_state_selector_keys, key=lambda x: float(x.split("_")[2]))

        for state_selector_key in sorted_state_selector_keys:
            # for state_selector in self.main:
            state_selector = self.states[state_selector_key]
            if state_selector.was_matched:
                continue
            if state_selector.match(state, last_state):
                matched_state_selector = state_selector
                state_selector.was_matched = True
                break
...
class ViewSelector(object):
    ...
    def parse(self):
        ...
        for selector_key in self.selector_dict:
            ...
            elif selector_key == 'condition':
                if selector_value == 'disappear':
                    self.disappear = True
            elif selector_key == 'xpath':
                self.xpath = selector_value

    def match_xpath(self, state):
        """
        return True if this view_selector's xpath matches the state
        @param state: DeviceState
        @return:
        """
        if not self.xpath:
        ...
        # parser = etree.XMLParser(remove_blank_text=True, remove_comments=True, encoding='utf-8', recover=True)
        # xml = etree.XML(state.xml, parser)
        # xml_header = "<?xml version='1.0' encoding='UTF-8' standalone='yes'?>"
        # state.xml = state.xml.replace(xml_header, "")
        index = state.xml.index("<hierarchy")
        state.xml = state.xml[index:]
        xml = etree.fromstring(state.xml)
        xpath_fit = self.revise_xpath(self.xpath)

        node = xml.xpath(xpath_fit)
        if node:
            if node[0].attrib['class'] and node[0].attrib['class'] != "":
                self.class_re = re.compile(node[0].attrib['class'])
            if node[0].attrib['bounds'] and node[0].attrib['bounds'] != "":
                coords = re.findall(r'\d+', node[0].attrib['bounds'])
                x1, y1, x2, y2 = map(int, coords)
                x, y = (x2 + x1) / 2, (y2 + y1) / 2
                self.in_coordinates.append([x, y])
            # print(f"xpath is a valid XPath in the XML")
            return True
        else:
            # print(f"xpath is not a valid XPath in the XML")
            return False

    def revise_xpath(self,xpath):
        # revise xpath from appium representation to uiautomator representation
        if '/hierarchy' in xpath:
            element_list = xpath.split("/")
    ...
    def match(self, view_dict):
        ...
        if self.content_desc_re and not safe_re_match(self.content_desc_re, view_dict['content_description']):
            return False
        if self.resource_id_re and safe_re_match(self.resource_id_re, "com.contextlogic.home:id/sign_in_fragment_email_text"):
            pass
    ...
    def match_last_state(self, device_state):
        ...
        for view_dict in view_dicts:
            if self.match(view_dict):
                view_selector_matched = True
                break
        return view_selector_matched
...
class StateSelector(object):
    ...
    def __init__(self, state_selector_id, selector_dict, script):
        ...
        self.views = set()
        self.parse()
        # add a tag to describe the state has been matched and has been used
    ...
    def match(self, device_state, last_state=None):
        ...
        for view_selector in self.views:
            view_selector_matched = False
            # match disappear oracle type view, now only consider the condition which 'len(views)==1'
            # if the view_selector is the disappeared oracle type, should match the last state first
            if view_selector.disappear:
                if not view_selector.match_last_state(last_state):
                    return False
                # init view_selector_matched with True
                view_selector_matched = True
            view_dicts = device_state.views
            if view_dicts is None:
                return False
            if not isinstance(view_dicts, list):
                return False
            if view_selector.xpath and view_selector.match_xpath(device_state):
                return True
            for view_dict in view_dicts:
                # if not view_selector.disappear and view_selector.match(view_dict):
                #     view_selector_matched = True
                #     break
                if not view_selector.disappear:
                    if not view_selector.xpath and view_selector.match(view_dict):
                        view_selector_matched = True
                        break
                # set False when current state not contain this view_selector
                if view_selector.disappear and view_selector.match(view_dict):
                    view_selector_matched = False
                    break
...
class ScriptEvent():
    ...
    def get_transformed_event(self, input_policy):
        ...
        if 'target_view' in event_dict:
            ...
            if event_dict['event_type'] == 'oracle' \
                    and 'condition' in event_dict and event_dict['condition'] == 'disappear':
                state = input_policy.last_state
            else:
            ...
            if state:
                ...
                if matched_view is None:
                    ...
                    # add xpath in view for only xpath
                    event_dict['view']['xpath'] = target_view_selector.xpath
