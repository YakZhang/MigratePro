...
class InputPolicy(object):
    ...
    def start(self, input_manager):
        ...
        action_max_count = 30
        while input_manager.enabled and (time.time() - start_time < 300):
            try:
                ...
                # self.generate_event() the state has target_vew
                if event == None:
                    end_time = time.time()
                    last_time = end_time - start_time
                    self.logger.info(
                        "Have finish the search and the action number is %s" % self.action_count)  # Todo: how to get info string
                    self.logger.info("save output_file is %s" % self.output_dir)
                    info_str = "Have finish the search and the action number is " + str(self.action_count)
                    time_str = "last time is " + str(last_time)
                    if self.output_dir is not None:
                        logger_info_file_name = "%s/logger_info.txt" % (self.output_dir)
        ...
        if self.action_count >= action_max_count:
            self.logger.info("Cannot finish the search within action number is %s" % action_max_count)
            self.logger.info("save output_file is %s" % self.output_dir)
            info_str = "Cannot finish the search within action number is " + str(action_max_count)
            end_time = time.time()
            last_time = end_time - start_time
            time_str = "last time is " + str(last_time)
            if self.output_dir is not None:
...
class UtgBasedInputPolicy(InputPolicy):
    ...
    def generate_event(self):
        ...
        # if the previous operation is finished, set operation['executed'] = True
        if len(self.script_events) > 0 and len(self.script_events) == self.script_event_idx:
            if self.current_operation is not None:  # for KeyEvent
                self.current_operation.executed = True

            # self.main[self.current_state][self.current_operation].execute = True

        # if all operations in script have been executed, check if current state has the specific view
        if self.script.is_all_operations_executed():
            event = None
            return event
            # my_views = []
            # my_view = ViewSelector(None, None, None)
            # for key in self.script_target_view:
            #     if 'resource_id' in self.script_target_view[key]:
            #         my_view.resource_id_re = re.compile(self.script_target_view[key]['resource_id'])
            #     if 'class' in self.script_target_view[key]:
            #         my_view.class_re = re.compile(self.script_target_view[key]['class'])
            #     if 'content_description' in self.script_target_view[key]:
            #         my_view.content_desc_re = re.compile(self.script_target_view[key]['content_description'])
            #     if 'text' in self.script_target_view[key]:
            #         my_view.text_re = re.compile(self.script_target_view[key]['text'])
            #     my_views.append(my_view)
            #
            # for view_selector in my_views:
            #     view_dicts = self.current_state.views
        ...
        if event is None and self.script is not None:
            operation = self.script.get_operation_based_on_state(self.current_state, self.last_state)
